...
public class ServiceGenerator extends CodeGeneratorManager implements CodeGenerator {
	...
	public void genCode(String tableName, String modelName, String sign) {
		...
		try {
			/*// 创建 Service 接口
			File serviceFile = new File(PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_BASE + PACKAGE_PATH_SERVICE + customMapping
			...
			// 创建 Service 接口的实现类
			File serviceImplFile = new File(OUTFILE_PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_BASE + PACKAGE_PATH_SERVICE_IMPL + modelNameUpperCamel + "Impl.java");
			// 查看父级目录是否存在, 不存在则创建
